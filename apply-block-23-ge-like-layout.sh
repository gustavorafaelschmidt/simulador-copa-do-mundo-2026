...
echo "==> Aplicando Bloco 23 — layout de grupos 3x e mata-mata espelhado no estilo GE..."
...
if [ ! -f "components/world-cup/VisualWorldCupSimulator.tsx" ]; then
  echo "ERRO: components/world-cup/VisualWorldCupSimulator.tsx não encontrado."
  exit 1
fi

mkdir -p .backup/block-23-ge-like-layout docs
cp components/world-cup/VisualWorldCupSimulator.tsx .backup/block-23-ge-like-layout/VisualWorldCupSimulator.tsx.backup
...
cat > docs/ge-like-layout.md <<'EOF'
# Bloco 23 — Layout inspirado no simulador GE

## Ajustes

- Grupos exibidos em três colunas no desktop.
- Cards de grupo com título central, linha superior, linhas cinza alternadas e botão verde.
- Mata-mata deixa de ser horizontal por colunas simples e passa para um desenho espelhado:
  - lado esquerdo;
  - centro com troféu/campeã;
  - lado direito.
- Mantidos rótulos pequenos por confronto, como `Jogo 76 | 1C × 2F`.

## Observação legal

O layout se aproxima da composição visual solicitada, mas não copia assets, logos, código-fonte ou imagens proprietárias.
EOF

echo "==> Bloco 23 aplicado."
echo ""
echo "Agora rode:"
echo "  npm run lint"
echo "  npm run test"
...
echo "  git status"
echo "  git add ."
echo "  git commit -m \"feat: align simulator layout with ge-style bracket\""
echo "  git push"
